...
export const updatePassword = async (req, res) => {
  ...
  try {
    const { currentPassword, newPassword, confirmNewPassword } = req.body;
      let changedNewpassword = await bcrypt.hash(req.body.newPassword, 12);
      let changedConfirmNewPassword = changedNewpassword;
    ...
    if (changedNewpassword !== changedConfirmNewPassword) {
      return res.status(400).json({
        error: "New passwords do not match",
      });
    }

    user.password = changedNewpassword;
    user.passwordConfirm = changedConfirmNewPassword;
    await user.save();
